...
class EngineTests(unittest.TestCase):
    ...
    def test_top_trending_invalid_bucket(self):
        with self.assertRaises(ValueError):
            self.engine.top_trending_content(limit=5, bucket="ночь")


    def test_performance_by_content_shape(self):
        ranked = self.engine.pick_next()
        self.engine.mark_posted(ranked.content_id, ranked.score, bucket="день", style="viral")
        self.engine.add_feedback(ranked.content_id, 8)
        rows = self.engine.performance_by_content(limit=5)
        self.assertGreaterEqual(len(rows), 1)
        self.assertIn("content_id", rows[0])
        self.assertIn("avg_feedback", rows[0])
