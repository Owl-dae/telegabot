...
async def audit_content(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    engine: ContentEngine = context.application.bot_data["engine"]
    cfg: BotConfig = context.application.bot_data["config"]
    if not await _admin_guard(update, cfg) or not update.message:
        return

    report = engine.audit_content()
    sample = "\n".join(report["issues_sample"]) if report["issues_sample"] else "Проблем не найдено"
    await update.message.reply_text(
        f"Content audit\nitems={report['total_items']}\nissues={report['issues_count']}\n\n{sample}"
    )


async def monetization_report(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    engine: ContentEngine = context.application.bot_data["engine"]
    if not update.message:
        return
    s = engine.stats()
    offer_url = _state(context, "offer_url", "")
    await update.message.reply_text(
        f"Monetization report\n"
        f"enabled={_state(context, 'monetization_enabled', False)}\n"
        f"offer_set={bool(offer_url)}\n"
        f"posts={s['total_posts']}\n"
        f"avg_pick={s['avg_pick_score']}\n"
        f"best_style={engine.recommend_best_style()}\n"
        f"best_bucket={engine.recommend_best_bucket()}"
    )
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        "/strategy /publish_plan /recent [n] /cleanup_data [days] /doctor\n"
        "/preview_id <id> [style] /top_performers [n]"
    )
...
async def doctor(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    report.append("✅ TELEGRAM_BOT_TOKEN set")
    report.append("✅ TELEGRAM_CHANNEL_ID set" if cfg.channel_id else "❌ TELEGRAM_CHANNEL_ID missing")
    report.append("✅ schedule configured" if cfg.schedule_times else "❌ schedule empty")

    if cfg.admin_ids:
        report.append(f"✅ ADMIN_IDS set ({len(cfg.admin_ids)})")
    else:
        report.append("⚠️ ADMIN_IDS not set (commands open to all users)")

    offer_url = str(_state(context, "offer_url", "")).strip()
    monet = bool(_state(context, "monetization_enabled", False))
    if monet and offer_url:
        report.append("✅ monetization enabled and offer configured")
    elif monet and not offer_url:
        report.append("⚠️ monetization enabled but OFFER_URL missing")
    else:
        report.append("ℹ️ monetization disabled")

    audit = engine.audit_content()
    if audit["issues_count"] == 0:
        report.append("✅ content audit clean")
    else:
        report.append(f"⚠️ content audit issues: {audit['issues_count']}")

    report.append(f"ℹ️ best bucket: {engine.recommend_best_bucket()}")
    report.append(f"ℹ️ best style: {engine.recommend_best_style()}")

    await _safe_reply(update, "\n".join(report))



async def preview_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    engine: ContentEngine = context.application.bot_data["engine"]
    cfg: BotConfig = context.application.bot_data["config"]
    if not await _admin_guard(update, cfg) or not update.message:
        return
    ...
    if not context.args:
        await _safe_reply(update, "Использование: /preview_id <content_id> [style]")
        return
    ...
    try:
        ...
        await _safe_reply(update, "Некорректный content_id")
        return

    style = context.args[1] if len(context.args) > 1 and context.args[1] in VALID_STYLES else str(_state(context, "post_style", "viral"))
    text = format_post(cid, item, style=style) + _build_monetization_block(context, cid, item.get("posting_time", "день"), style)
    await _safe_reply(update, text, parse_mode="HTML", reply_markup=_feedback_buttons(cid))


async def top_performers(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    engine: ContentEngine = context.application.bot_data["engine"]
    cfg: BotConfig = context.application.bot_data["config"]
    if not await _admin_guard(update, cfg) or not update.message:
        return
    ...
    rows = engine.performance_by_content(limit=n)
    if not rows:
        await _safe_reply(update, "Нет данных по производительности")
        return

    lines = [f"Top performers (n={n}):"]
    for i, r in enumerate(rows, 1):
        lines.append(
            f"{i}. #{r['content_id']} {r['title']} | posts={r['posts_count']} | avg_feedback={r['avg_feedback']} | avg_pick={r['avg_pick_score']}"
        )
    await _safe_reply(update, "\n".join(lines))
...
def main() -> None:
    ...
    app.add_handler(CommandHandler("preview_id", preview_id))
    ...
    app.add_handler(CommandHandler("top_performers", top_performers))
    app.add_handler(CommandHandler("trending", trending))
    app.add_handler(CommandHandler("audit_content", audit_content))
    app.add_handler(CommandHandler("monetization_report", monetization_report))
