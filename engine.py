...
class ContentEngine:
    ...
    def _load_content(self) -> List[Dict[str, Any]]:
        ...
        for idx, item in enumerate(raw):
            ...
            if posting_time not in VALID_BUCKETS:
    ...
    def _metrics_snapshot(self, recency_hours: int = 72) -> tuple[dict[int, float], dict[int, int], set[int], set[int]]:
        with sqlite3.connect(self.db_path) as conn:
            feedback_rows = conn.execute(
                "SELECT content_id, AVG(value) FROM feedback GROUP BY content_id"
            ).fetchall()
            post_rows = conn.execute(
                "SELECT content_id, COUNT(*) FROM posts GROUP BY content_id"
            ).fetchall()
    ...
    def rank_candidates(
        ...
        feedback_avg, posts_count, recent_ids, cooldown_ids = self._metrics_snapshot()
    ...
    def recommend_best_style(self, bucket: Optional[str] = None) -> str:
        with sqlite3.connect(self.db_path) as conn:
            if bucket in VALID_BUCKETS:
                rows = conn.execute(
                    """
                    SELECT style, AVG(score) avg_score
                    FROM posts
                    WHERE style IS NOT NULL AND bucket = ?
                    GROUP BY style
                    """,
                    (bucket,),
                ).fetchall()
                ...
                rows = conn.execute(
                    "SELECT style, AVG(score) avg_score FROM posts WHERE style IS NOT NULL GROUP BY style"
                ).fetchall()
    ...
    def stats(self) -> Dict[str, Any]:
        with sqlite3.connect(self.db_path) as conn:
            ...
            top = conn.execute(
                """
                SELECT content_id, AVG(value) avg_score, COUNT(*) c
                FROM feedback GROUP BY content_id
                ORDER BY avg_score DESC, c DESC LIMIT 5
                """
            ).fetchall()
