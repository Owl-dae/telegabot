...
class ContentEngine:
    ...
    def performance_by_content(self, limit: int = 10) -> List[Dict[str, Any]]:
        limit = max(1, min(100, int(limit)))
        with sqlite3.connect(self.db_path) as conn:
            rows = conn.execute(
                """
                SELECT
                    ...
                    AVG(p.score) AS avg_pick_score,
                    COALESCE((SELECT AVG(f.value) FROM feedback f WHERE f.content_id = p.content_id), 0) AS avg_feedback
                FROM posts p
                GROUP BY p.content_id, p.title
                ORDER BY avg_feedback DESC, avg_pick_score DESC, posts_count DESC
                LIMIT ?
                """,
                (limit,),
            ).fetchall()
        ...
        return [
            {
                "content_id": r[0],
                "title": r[1],
                "posts_count": int(r[2] or 0),
                "avg_pick_score": round(float(r[3] or 0), 2),
                "avg_feedback": round(float(r[4] or 0), 2),
            }
            for r in rows
        ]
